...
<tr class="product_row" data-row_index="{{ $row_count }}" @if (!empty($so_line)) data-so_id="{{ $so_line->transaction_id }}" @endif>
    ...
    " style="height:50px; width:50px; object-fit:cover">
        ...
        <textarea class="form-control" placeholder="Note" name="products[{{ $row_count }}][product_sele_note]" rows="1">{{ $sell_line_note ?? '' }}</textarea>
    </td>
    ...
    <td>
        ...
        <div class="input-group input-number">
            ...
            <input type="text" data-min="1" class="form-control pos_quantity input_number mousetrap input_quantity" value="{{ @format_quantity($product->quantity_ordered) }}"
        ...
        @if ($product->product_type == 'combo' && !empty($product->combo_products))
            ...
            @foreach ($product->combo_products as $k => $combo_product)
                ...
                <input type="hidden" class="combo_product_qty" name="products[{{ $row_count }}][combo][{{ $k }}][quantity]" data-unit_quantity="{{ $combo_product['qty_required'] }}"
                    value="{{ $qty_total }}">
    ...
    <td class="text-center v-center">
        ...
        <span class="text-link text-info cursor-pointer getproduct" data-product_id="{{ $product->product_id }}" data-variation_id="{{ $product->variation_id }}" data-toggle="modal"
            data-target="#bd_transaction_modal_lg_{{ $row_count }}">
            <i class="fa fa-eye"> </i>
        </span>

        <i class="fa fa-times text-danger pos_remove_row cursor-pointer" aria-hidden="true"></i>
        <!-- transactions_modal.blade.php -->
        <div class="modal fade row_edit_product_price_model_h" id="bd_transaction_modal_lg_{{ $row_count }}" tabindex="-1" role="dialog">
            @include('sale_pos.partials.transactions_modal');
        </div>
